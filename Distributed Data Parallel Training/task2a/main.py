...
def train_model(model, train_loader, optimizer, criterion, epoch, world_size):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        my_world = td.group.WORLD
	# perform scatter and gather
        for p in model.parameters():
            my_gather = []
            my_means = []
            # perform gathering tensors first if any
            if td.get_rank() == 0:
                my_gather = [torch.zeros_like(p.grad.data) for i in range(4)]
                td.gather(p.grad.data, my_gather, group = my_world)
                ans = torch.add(my_gather[0], my_gather[1], alpha = 1)
                ans = torch.add(ans, my_gather[2], alpha = 1)
                ans = torch.add(ans, my_gather[3], alpha = 1)
                ans = ans / 4
                my_means = [ans for i in range(4)]
            else:
                td.gather(p.grad.data, dst = 0, group = my_world)

            # perform scattering
            if td.get_rank() == 0:
                td.scatter(tensor = p.grad.data, scatter_list = my_means, src = 0, group = my_world)
            else:
                td.scatter(tensor = p.grad.data, src = 0, group = my_world)
